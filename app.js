...
app.configure(function(){
  ...
  app.use(express.cookieParser('Thoughtpivot'));
  app.use(express.cookieSession());
  app.use(bodyParser.json());
  app.use(bodyParser.urlencoded({ extended: true }));
  app.use(session({secret: 'Thoughtpivot', cookie: { maxAge: 60000 * 30 }}));
...
app.get('/activation/:name', function (req, res) 
{
  if (req.params.name) 
  {
    res.render('activation', { title: 'Activation', name: req.params.name })
  }
  res.redirect('/')
});
...
app.get('/dashboard/:name', function (req, res)
{
  ...
  res.render('dashboard', { title: 'Dashboard of ' + req.params.name + '', name: req.params.name })
})
  ...
  if (req.params.name && req.session.name) 
  {
    if (req.params.name === req.session.name) 
    {
      DB.keys().forEach(function(id)
      {
        ...
        {
          res.render('profile', { title: 'Profile of ' + req.params.name + '', name: req.params.name, account: account })
        }
      })
    }
  }
...
app.get('/azure/asm/account/list', function (req, res)
{
    ...
    var azure = "azure account list --json";
...
app.put('/azure/asm/account/set/:tenantId', function (req, res)
{
    ...
    var azure = "azure account set " + req.params.tenantId + " --json";
...
app.get('/azure/asm/service/list', function (req, res, next)
{
  ...
  var azure = "azure service list --json";
...
app.get('/azure/asm/service/show/:serviceName', function (req, res)
{
    ...
    var azure = "azure service show --serviceName " + req.params.serviceName + " --json";
...
app.get('/azure/arm/group/list', function (req, res)
{
    ...
    var azure = "azure group list --json";
...
app.get('/azure/arm/resource/list/:name', function (req, res)
{
    ...
    var azure = "azure resource list " + req.params.name + " --json";
